 ...
 const [disable,setDisable] = useState(false)
  const sendEmail = (e) => {
    ...
    if (email.length > 0 && fullName.length > 0) {
      emailjs.send("service_aqwq1mj", "template_8a96l8e", {
        from_name: fullName,
        message: message,
        email: email,
      }).then(() => {
        ...
        <Inner>
          ...
          <Body>
            <Form onSubmit={sendEmail}>
              ...
              <button disabled={disable} onClick={sendEmail}>Send</button>
            </Form>
            <Information>
              <Info>
                <svg
...
const Inner = styled.div`
  ...
  background: linear-gradient(
    114.11deg,
    rgba(255, 255, 255, 0.2) 8.75%,
    rgba(255, 255, 255, 0) 96.69%
  );
  border-radius: 10px;
